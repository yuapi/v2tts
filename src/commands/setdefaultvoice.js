...
export default {
  data: new SlashCommandBuilder()
    .setName('setdefaultvoice')
    .setDescription('서버 기본 TTS 화자를 변경합니다. (관리자 전용)')
    .addIntegerOption(opt =>
      opt.setName('speaker_id')
        .setDescription('화자 ID — /voices 로 목록 확인')
        .setRequired(true)
        .setMinValue(0)
    )
  ...
  async execute(interaction) {
    ...
    await interaction.editReply(`✅ 서버 기본 화자 ID가 **${id}**로 변경되었습니다.`);
  },
};
